  ...
  <div class="min-h-screen bg-[#0a0a0a] text-white py-10 px-6 font-sans pt-28">
    <div class="max-w-3xl mx-auto">
      ...
      <div class="bg-[#151515] p-4 rounded-2xl border border-gray-800 flex gap-2 mb-8">
        <input 
          v-model="telepon" type="text" placeholder="Masukkan No. WA..." 
        ...
        />
        <button @click="getRiwayat" class="bg-rose-600 text-white px-6 py-2 rounded-xl hover:bg-rose-700 transition font-bold uppercase text-sm tracking-wider">
          Cari
        </button>
      </div>

      <div v-if="loading" class="text-center py-10"><div class="animate-spin h-8 w-8 border-2 border-rose-600 rounded-full border-t-transparent mx-auto"></div></div>
      <div v-if="!loading && riwayat.length === 0 && sudahCari" class="text-center text-gray-500 py-10 bg-[#151515] rounded-2xl border border-gray-800 border-dashed">Belum ada riwayat ditemukan.</div>

      <div v-for="trx in riwayat" :key="trx.id" class="bg-[#151515] rounded-3xl border border-gray-800 overflow-hidden mb-8 hover:border-gray-600 transition">
        ...
        <div class="p-6">
          <div class="flex flex-col md:flex-row justify-between mb-6">
            <div>
              ...
              <p class="font-medium text-gray-300">{{ new Date(trx.created_at).toLocaleDateString('id-ID', { weekday:'long', year:'numeric', month:'long', day:'numeric'}) }}</p>
            </div>
          ...
          <div class="bg-black/50 rounded-xl p-4 space-y-3 mb-6 border border-gray-800">
            <div v-for="item in trx.items" :key="item.id" class="flex justify-between items-center border-b border-gray-800 last:border-0 pb-2 last:pb-0">
               <div>
                 <p class="font-bold text-gray-300">{{ item.nama_alat }} <span class="text-xs text-gray-500">x{{ item.jumlah }}</span></p>
                 <p class="text-[10px] text-gray-600 font-mono">{{ item.tanggal_mulai }} s/d {{ item.tanggal_selesai }}</p>
               </div>
               <p class="text-sm font-bold text-gray-400">Rp {{ item.subtotal.toLocaleString() }}</p>
            </div>
          </div>

          <button @click="kirimWA(trx)" class="w-full border border-green-800 bg-green-900/10 text-green-500 hover:bg-green-900/20 py-3 rounded-xl font-bold transition flex justify-center items-center gap-2 uppercase text-sm tracking-wider">
...
onMounted(() => {
  ...
  if (phone) { telepon.value = phone; getRiwayat(); }
});

const getRiwayat = async () => {
  if (!telepon.value) return alert("Isi nomor WA dulu");
  loading.value = true; sudahCari.value = true;
  try {
    const res = await axios.get(`http://127.0.0.1:8000/api/riwayat/${telepon.value}`);
