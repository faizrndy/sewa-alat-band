    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-1">
        <div class="bg-[#151515] p-6 rounded-3xl border border-gray-800 sticky top-28">
          ...
          <button @click="kirimPembayaran" :disabled="loading" class="w-full bg-white text-black py-4 rounded-xl font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_20px_rgba(225,29,72,0.5)] transition-all disabled:bg-gray-800 disabled:text-gray-600">
...
<script setup>
// LOGIC SAMA PERSIS, HANYA STYLE YANG BERUBAH
...
const kirimPembayaran = async () => {
  if (!itemsKeranjang.value.length) return alert("Keranjang kosong.");
  if (!nama.value || !telepon.value) return alert("Lengkapi data diri.");
  if (!fileIdentitas.value) return alert("Upload identitas.");
  if (metodePengiriman.value === 'antar' && (!lat.value)) return alert("Pilih lokasi di map.");
  if (jarak.value > 30 && metodePengiriman.value === 'antar') return alert("Kejauhan bos.");
  ...
  try {
    ...
    const res = await axios.post("http://127.0.0.1:8000/api/transaksi", form, { headers: { "Content-Type": "multipart/form-data" } });
    
    const snapToken = res.data.snap_token;
    if (window.snap && snapToken) {
      window.snap.pay(snapToken, {
        onSuccess: () => { localStorage.removeItem("cart"); window.location.href = `/riwayat?telepon=${telepon.value}`; },
        onPending: () => { window.location.href = `/riwayat?telepon=${telepon.value}`; },
        onError: () => alert("Gagal bayar"),
        onClose: () => alert("Ditutup")
      });
    } else {
