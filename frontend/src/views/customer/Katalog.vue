    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div>
            ...
            <select v-model="kategori" class="input-dark">
              ...
              <option>Gitar</option><option>Bass</option><option>Drum</option><option>Keyboard</option><option>Mikrofon</option><option>Sound</option>
            </select>
          </div>
          <div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <router-link
          ...
          :key="item.id"
          :to="`/katalog/${item.id}`"
          class="group bg-[#151515] rounded-3xl overflow-hidden border border-gray-800 hover:border-rose-600/50 hover:shadow-[0_0_30px_rgba(225,29,72,0.15)] transition-all duration-300 relative top-0 hover:-top-2 flex flex-col"
        >
          <div class="relative h-64 bg-[#0a0a0a] p-6 flex items-center justify-center overflow-hidden border-b border-gray-800">
            ...
            <div class="absolute top-3 left-3">
               <span :class="item.status === 'Tersedia' ? 'bg-green-500 text-black' : 'bg-red-600 text-white'" class="text-[10px] font-black px-3 py-1 uppercase tracking-widest rounded-full shadow-lg">
                 {{ item.status }}
               </span>
            </div>
          </div>
          ...
          <div class="p-6 flex flex-col flex-grow">
            ...
            <h3 class="text-white text-lg font-black uppercase italic leading-tight mb-4 line-clamp-2">{{ item.nama_alat }}</h3>
...
<style scoped>
.input-dark {
  @apply w-full bg-[#050505] border border-gray-700 text-white px-4 py-3 rounded-xl focus:border-rose-600 focus:ring-1 focus:ring-rose-600 outline-none transition placeholder-gray-600 h-[50px];
}
</style>